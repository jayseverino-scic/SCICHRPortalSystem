(function ($) {
    ...
    let attachEvents = () => {
        ...
        $('#position-form').on('submit', onFormSubmit);
    };

    let onClickAddModal = function () {
        $('#position-form')[0].reset();
        $('#position-form').find(':submit').text('Add');
    ...
    let onFormSubmit = async event => {
        ...
        if ($(event.target).valid()) {
            ...
            if (response.ok) {
                $('#position-grid').DataTable().ajax.reload(null, false);
    ...
    let initializeGrid = async () => {
        ...
        let table = $('#position-grid').DataTable({
            bLengthChange: true,
            ...
            ajax: async function (params, success, settings) {
                let gridInfo = $('#position-grid').DataTable().page.info();
                let searchKeyword = params.search.value;
                let pageSize = params.length;
                let response = await _apiHelper.get({
                    url: `Authenticated/Position/Filter?pageNumber=${gridInfo.page + 1}&pageSize=${pageSize}&searchKeyword=${searchKeyword}`,
                });
                ...
                if (response.ok) {
                    ...
                    $('#position-grid tbody').on('click', '.icon-edit', function () {
                    ...
                    $('.icon-delete').on('click', function (e) {
                        var data = table.row($(this).closest('tr')).data();
                        _formHelper.deleteRecord(e, data.positionName,SYSTEM);
                    });
                } else {
                    success(null);
                }
    ...
    let getColumns = async () => {
        ...
        let lastColumn = {
            ...
            render: function (data, type, full) {
                let buttons = '<a href="#" class="m-1 icon-edit" data-id="' + full.positionId + '" data-endpoint="Authenticated/Position" data-table="position-grid"><i class="fas fa-edit"></i></a>';
                buttons += '<a href="#" class="m-1 icon-delete" data-id="' + full.positionId + '" data-endpoint="Authenticated/Position" data-table="position-grid"><i class="fas fa-trash border-icon"></i></a>';
