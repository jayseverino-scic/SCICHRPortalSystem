(function ($) {
    ...
    let onClickAddModal = function () {
        ...
        $('#project-form').find(':submit').text('Add');
    ...
    let onClickDbFilter = async e => {
        ...
        let response = await _apiHelper.get({
            url: `Authenticated/Project/ImportDb`,
        });
        ...
        if (response.ok) {
            ...
            if ($.fn.DataTable.isDataTable('#project-grid')) {
                $('#project-grid').DataTable().ajax.reload(null, false);
            } else {
                initializeGrid();
            }
        }
    ...
    let initializeGrid = async () => {
        ...
        let table = $('#project-grid').DataTable({
            bLengthChange: true,
            ...
            ajax: async function (params, success, settings) {
                let gridInfo = $('#project-grid').DataTable().page.info();
                let searchKeyword = params.search.value;
                let pageSize = params.length;
                let response = await _apiHelper.get({
                    url: `Authenticated/Project/Filter?pageNumber=${gridInfo.page + 1}&pageSize=${pageSize}&searchKeyword=${searchKeyword}`,
                });
                ...
                if (response.ok) {
                    ...
                    $('.icon-delete').on('click', function (e) {
                        var data = table.row($(this).closest('tr')).data();
                        _formHelper.deleteRecord(e, data.projectName, SYSTEM);
                    });
                } else {
                    success(null);
                }
    ...
    let getColumns = async () => {
        ...
        let lastColumn = {
            ...
            render: function (data, type, full) {
                let buttons = '<a href="#" class="m-1 icon-edit" data-id="' + full.projectId + '" data-endpoint="Authenticated/Project" data-table="project-grid"><i class="fas fa-edit"></i></a>';
                buttons += '<a href="#" class="m-1 icon-delete" data-id="' + full.projectId + '" data-endpoint="Authenticated/Project" data-table="project-grid"><i class="fas fa-trash border-icon"></i></a>';
