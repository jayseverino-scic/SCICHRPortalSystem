(function ($) {
    ...
    let initializeGrid = async () => {
        ...
        let table = $('#device-grid').DataTable({
            bLengthChange: true,
            ...
            ajax: async function (params, success, settings) {
                let gridInfo = $('#device-grid').DataTable().page.info();
                let searchKeyword = params.search.value;
                let pageSize = params.length;
                let response = await _apiHelper.get({
                    url: `Authenticated/Device/Filter?pageNumber=${gridInfo.page + 1}&pageSize=${pageSize}&searchKeyword=${searchKeyword}`,
                });
                ...
                if (response.ok) {
                    ...
                    $('#device-grid tbody').on('click', '.icon-edit', function () {
                        var data = table.row($(this).closest('tr')).data();
                        let form = $('#device-form');
                    ...
                    $('.icon-delete').on('click', function (e) {
                        var data = table.row($(this).closest('tr')).data();
                        _formHelper.deleteRecord(e, data.deviceName, SYSTEM);
                    });
                } else {
                    success(null);
                }
    ...
    let getColumns = async () => {
        ...
        let lastColumn = {
            ...
            render: function (data, type, full) {
                let buttons = '<a href="#" class="m-1 icon-edit" data-id="' + full.deviceId + '" data-endpoint="Authenticated/Device" data-table="device-grid"><i class="fas fa-edit"></i></a>';
                buttons += '<a href="#" class="m-1 icon-delete" data-id="' + full.deviceId + '" data-endpoint="Authenticated/Device" data-table="device-grid"><i class="fas fa-trash border-icon"></i></a>';
