...
namespace SCICHRPortal.API.Controllers.Authenticated
{
    ...
    [ApiController]
    public class XCompanyPositionController : ControllerBase
    {
        private IXCompanyPositionService CompanyPositionService { get; }
        public XCompanyPositionController(IXCompanyPositionService companyPositionService)
        {
        ...
        {
            var position = await CompanyPositionService.GetAllAsync();
            return Ok(position);
        }
        ...
        {
            var tuple = await CompanyPositionService.FilterAsync(pageNumber, pageSize, searchKeyword!);
