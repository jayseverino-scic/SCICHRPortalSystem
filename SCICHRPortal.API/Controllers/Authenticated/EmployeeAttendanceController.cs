...
namespace SCICHRPortal.API.Controllers.Authenticated
{
    ...
    public class EmployeeAttendanceController : ControllerBase
    {
        ...
        public EmployeeAttendanceController(IEmployeeAttendanceService employeeAttendanceService, IEmployeeService employeeService, IShiftService shiftService, IDepartmentService departmentService, IEmployeeTimeLogService employeeTimeLogService, ITimekeepingAdminSetupService timekeepingAdminSetupService, ICutOffService cutOffService)
        {
            ...
            foreach (var employee in employees)
            {
                ...
                if (perEmployeeAttendance.Any())
                {
                    ...
                    foreach(var attendance in perEmployeeAttendance)
                    {
                        ...
                        holidayTotalHours += attendance.ApprovedHoliday ? attendance.RegularHour : 0;
                        holidayOvertimeTotalHours += attendance.ApprovedHolidayOT ? attendance.OTHours : 0;
                        holidayNightDifferentialTotalHours += attendance.ApprovedHoliday ? attendance.NDHours : 0;
                        specialHolidayTotalHours += attendance.ApprovedSPHoliday ? attendance.RegularHour : 0;
                        specialHolidayOvertimeTotalHours += attendance.ApprovedSPHolidayOT ? attendance.OTHours : 0;
                        specialHolidayNightDifferentialTotalHours += attendance.ApprovedSPHoliday ? attendance.NDHours : 0;
                        restDayTotalHours += attendance.ApprovedRestDay ? attendance.RegularHour : 0;
                        restDayOvertimeTotalHours += attendance.ApprovedRestDayOT ? attendance.OTHours : 0;
        ...
        public async Task<IActionResult> EmployeeAttendanceFilterAsync(int departmentId, DateTime attendanceDate)
        {
                ...
                foreach (EmployeeTimeLog employee in mergedList)
                {
                    ...
                    if (duplicatedLog == null)
                    {
                        EmployeeAttendance employeeAttendance = new EmployeeAttendance
                        {
                            ...
                            BreakIn = (DateTime)employee.BreakIn!.Value,
                            BreakOut = (DateTime)employee.BreakOut!.Value,
            ...
            foreach (EmployeeAttendance employeeAttendance in employeeAttendances.ToList())
            {
                EmployeeAttendanceUpdateRequestModel employeeAttendanceUpdateRequestModel = new EmployeeAttendanceUpdateRequestModel
                {
                    ...
                    BreakIn = employeeAttendance.BreakIn,
                    BreakOut = employeeAttendance.BreakOut,
                    ShiftStart = employeeAttendance.ShiftStart,
                    ShiftEnd = employeeAttendance.ShiftEnd,
                    BreakStart= employeeAttendance.BreakStart,
                    BreakEnd= employeeAttendance.BreakEnd,
        ...
        public async Task<IActionResult> UpdateShiftAssignmentAsync(List<EmployeeAttendanceUpdateRequestModel> employeeAttendance)
        {
            ...
            foreach (var item in employeeAttendance)
            {
                ...
                EmployeeAttendance attendance = new EmployeeAttendance
                {
                    ...
                    ShiftUndertime = ComputeHours(item.TimeOut, item.ShiftEnd.Value),
                    ShiftLate = ComputeHours(item.TimeIn, item.ShiftStart.Value),
                    BreakUndertime = ComputeHours(item.BreakOut!.Value, item.BreakStart!.Value),
                    BreakLate = ComputeHours(item.BreakOut!.Value, item.BreakEnd!.Value.AddMinutes(timekeepingAdminSetup.BreakLateMinuteGracePeriod)),
